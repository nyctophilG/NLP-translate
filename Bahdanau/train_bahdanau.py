...
HIDDEN_SIZE = 256
BATCH_SIZE = 32
LEARNING_RATE = 0.001 # Reduced slightly for stability
DROPOUT = 0.1
EPOCHS = 10 # Start with 10 for quick feedback
PRINT_EVERY = 100
MAX_LENGTH = 15
...
def train_epoch(dataloader, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion):
    ...
    for i, data in enumerate(dataloader):
        ...
        # In a real scenario, we might want scheduled sampling, but for this assignment simpler is safer.
        decoder_outputs, _, _ = decoder(encoder_outputs, encoder_hidden, target_tensor)

        # Flatten for CrossEntropyLoss
        # decoder_outputs: [batch, target_len, vocab_size]
        # target_tensor: [batch, target_len]
        
        # Ensure we only calculate loss for the valid length
        output_dim = decoder_outputs.shape[-1]
        
        # Safety check: if decoder output length differs from target (shouldn't happen with force), slice match
...
def train(train_dataloader, encoder, decoder, n_epochs, learning_rate=0.001, print_every=100):
    ...
    criterion = nn.CrossEntropyLoss(ignore_index=PAD_token) # Ignore padding if we had it, or just standard
...
if __name__ == '__main__':
    ...
    # For simplicity in this script, we'll use batch_size=1 (Stochastic Gradient Descent style)
    # or implement a collate_fn for batching. 
    # Let's use batch_size=1 to be safe with variable lengths without complex padding logic right now,
    # or just use our collate_fn with padding 0.
    
    from torch.nn.utils.rnn import pad_sequence
    def collate_batch(batch):
        input_tensors, target_tensors = zip(*batch)
        # Pad with 0 (which we mapped to PAD in Lang, wait. In utils.py, 0 is PAD? Yes, 0=PAD, 1=SOS, 2=EOS)
    ...
    # 4. Save Models
    torch.save(encoder.state_dict(), 'encoder_bahdanau.pth')
    torch.save(decoder.state_dict(), 'decoder_bahdanau.pth')
    print("Models saved.")
